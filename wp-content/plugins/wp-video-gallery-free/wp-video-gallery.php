...
function wp_video_gallery_connect() {
	$lib = new \Vimeo\Vimeo('', '');
	$lib->setToken(WP_VIDEO_GALLERY_VIMEO_AT);
	return $lib;
}
...
function wp_video_gallery_refresh_database() {
	...
	while ($i <= $total && $q && ($qo < 50 || $activated)) {
		$response = $lib->request('/me/videos', array('per_page' => $per_page, 'sort' => 'modified_time', 'direction' => 'desc', 'page' => $i), 'GET');
		$total = (int)((int)$response["body"]["total"] / $per_page) + 1;
		$i++;
...
function wp_video_gallery_vimeo_add_menu() {
	...
	if ($activated)
		...
		add_menu_page('Japo Genki Video Gallery', 'Japo Genki Video Gallery', 'manage_options', 'wp_video_gallery-edit-grids', 'wp_video_gallery_edit_grids',  plugin_dir_url(__FILE__).'img/menu-logo.png');
...
wp_register_style('wp_video_gallery-front-css', plugin_dir_url(__FILE__) . 'front/wp_video_gallery-front-css.css');
...
wp_register_script('majba-scripts', plugin_dir_url(__FILE__) . 'js/scripts.js');
wp_localize_script('majba-scripts', 'preview_path', plugin_dir_url(__FILE__) . 'js/preview.js');

wp_register_script('majba-add-grid-scripts', plugin_dir_url(__FILE__) . 'js/add-grid.js');
...
wp_register_script('wp_video_gallery-front-js', plugin_dir_url(__FILE__) . 'front/wp_video_gallery-front-js.js');
...
function wp_video_gallery_days_html() {
	...
	$o = '
		<div id="wp_video_gallery_daez">
			' . __("You can manage up to 50 videos with the WP Video Gallery free version. For a professional and unrestricted use of WP Video Gallery, <a href='http://wp_video_gallery-plug.in/' target='_blank'>Get the premium version</a>", 'wp_video_gallery') . '
		</div>
	';
	return $o;
}
...
function wp_video_gallery_days_html_ex() {
	...
	$o = '
		<div id="wp_video_gallery_daez_ex">
			' . __("You have already reached your 50 videos limit. For a professional and unrestricted use of this plugin, <a href='http://wp_video_gallery-plug.in/' target='_blank'>Get the premium version</a>", 'wp_video_gallery') . '
		</div>
	';
	return $o;
}
...
function get_youtube_public_data($url) {
	...
	$content = file_get_contents("http://youtube.com/get_video_info?video_id=".$query ['v']);
...
function check_youtube_api_key($key) {
	$content = file_get_contents("https://www.googleapis.com/youtube/v3/videos?part=snippet&id=VA770wpLX-Q&key=" . $key);
	if (!$content)
		return 0;
	return 1;
}
