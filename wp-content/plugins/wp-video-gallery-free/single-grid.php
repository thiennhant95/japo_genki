...
function wp_video_gallery_edit_grid($gridId) {
		...
		echo '
						<div class="brick ' . $vimeo_class . ' chosen item-' . $id . '" data-id="' . $id . '">
							...
							<div class="bigContainer">
								<span>' . __('Mosaic', 'wp_video_gallery') . '</span>
								<input type="checkbox" ' . $checked . ' class="isBig">
		...
		if (!in_array($value->uri, $selectedVideosUris)) {
			...
			echo '
						<div class="brick ' . $vimeo_class . ' item-' . $id . '" data-id="' . $id . '">
							...
							<div class="bigContainer">
								<span>' . __('Mosaic', 'wp_video_gallery') . '</span>
								<input type="checkbox" class="isBig">
								<i class="fa fa-times"></i>
								<i class="fa fa-bars"></i>
							</div>
						</div>
	...
	';
	$allTagsChecked = count($gridConf['filters']) > 0 ? '' : " checked";
	echo '
							<div class="check-all-tags '.count($gridConf['filters']).'" data-filter="all-tags">
	...
	$display_trigger_0 = (int)$gridConf['display-trigger'] == 0 ? 'selected' : '';
	$display_trigger_1 = (int)$gridConf['display-trigger'] == 1 ? 'selected' : '';
	...
	echo '		<div class="themeList accordion open">
					...
					<div class="inAccordion">
						<form id="themeForm">
						    <input type="radio" name="theme" value="Classic"' . $whichTheme[0] . '/>' . __('Classic', 'wp_video_gallery') . '<br>
						    <input type="radio" name="theme" value="Mosaic"' . $whichTheme[1] . '/>' . __('Mosaic', 'wp_video_gallery') . '<br>
						    <input type="radio" name="theme" value="Slider"' . $whichTheme[2] . '/>' . __('Slider', 'wp_video_gallery') . '
						</form>
					</div>
				</div>
				<div class="loadingBox accordion close">
					...
					<div class="inAccordion">
						<div>
							<label>' . __("How do you want to trigger the gallery's display ?", 'wp_video_gallery') . ' :</label>
							<select class="display-trigger">
								<option value="0" ' . $display_trigger_0 . '>' . __('On page load', 'wp_video_gallery') . '</option>
								<option value="1" ' . $display_trigger_1 . '>' . __('On scroll to gallery', 'wp_video_gallery') . '</option>
							</select>
							<div class="clearfix"></div>
						</div>
						<div class="clearfix"></div>
				...
				<div class="titleBox accordion close">
					...
					<div class="inAccordion">
						<div>
							<label>' . __("Text's color", 'wp_video_gallery') . ' :</label>
							<input id="colorSelector1" class="colorSelector" value="' . $text_color . '">
							<div class="clearfix"></div>
						</div>
						<div>
							<label>' . __('Text color on hover', 'wp_video_gallery') . ' :</label>
				...
				<div class="tagsList accordion close" ' . $hideConfig[0] . '>
					...
					<div class="inAccordion">
						<form id="showTags">
						    ...
						    <input type="radio" name="tags" value="0"' . $showTag[0] . '/>' . __('No', 'wp_video_gallery') . '<br>
						    <input type="radio" name="tags" value="1"' . $showTag[1] . '/>' . __('Top', 'wp_video_gallery') . '<br>
						    <input type="radio" name="tags" value="2"' . $showTag[2] . '/>' . __('Left', 'wp_video_gallery') . '<br>
						    <input type="radio" name="tags" value="3"' . $showTag[3] . '/>' . __('Right', 'wp_video_gallery') . '
						</form>
				...
				<div class="configList accordion close" ' . $hideConfig[0] . '>
					...
					<div class="inAccordion">
						<div class="videoWidth">
							<p>' . __('Choose the width of the base element', 'wp_video_gallery') . ' :</p>
							<div id="videoSimpleWidth"><span>' . __('The width :', 'wp_video_gallery') . '</span><input onkeypress="return event.charCode >= 48 && event.charCode <= 57" type="text" value="' . $video_width . '"><span>px.</span></div>
							<p>' . __('Space between elements', 'wp_video_gallery') . ' :</p>
				...
				<div class="animationList accordion close" ' . $hideConfig[0] . '>
					...
					<div class="inAccordion">
						<div class="CSSAnimation videoWidth">
							<p>' . __('Blocks delay', 'wp_video_gallery') . ' :</p>
							<input class="animation_delay" onkeypress="return event.charCode >= 48 && event.charCode <= 57" type="text" value="' . $animation_delay . '">
							...
							<p>' . __('Animation effect', 'wp_video_gallery') . ' :</p>
					...
					<div class="inAccordion">
						<div id="sliderOption">
						    ...
						    <form class="formTitle">
						    	<p>' . __('Show the title', 'wp_video_gallery') . ' :</p>
						    	<input type="radio" name="showTitle" value="0"' . $showTitle[0] . '/>' . __('No', 'wp_video_gallery') . '<br>
					    		<input type="radio" name="showTitle" value="1"' . $showTitle[1] . '/>' . __('Yes', 'wp_video_gallery') . '
						    </form>
						    <form class="formPage">
						    	<p>' . __('Show the pagination', 'wp_video_gallery') . ' :</p>
						    	<input type="radio" name="showPage" value="0"' . $showPage[0] . '/>' . __('No', 'wp_video_gallery') . '<br>
					    		<input type="radio" name="showPage" value="1"' . $showPage[1] . '/>' . __('Yes', 'wp_video_gallery') . '
						    </form>
						</div>
					</div>
				</div>
		...
		if (isset($modalTags[$i])) {
			foreach ($gridConf['tags'] as $tag) {
				if ($tag[0] == $modalTags[$i] && $tag[1] == 0) {
					echo '<td><input type="checkbox"><span>' . $modalTags[$i] . '</span></td>';
					$j = 1;
				}
			}
			if ($j == 0)
				echo '	<td><input type="checkbox" checked><span>' . $modalTags[$i] . '</span></td>';
			$j = 0;
		}
		if (isset($modalTags[$i + $nn])) {
			foreach ($gridConf['tags'] as $tag) {
				if ($tag[0] == $modalTags[$i + $nn] && $tag[1] == 0) {
					echo '<td><input type="checkbox"><span>' . $modalTags[$i + $nn] . '</span></td>';
					$j = 1;
				}
			}
			if ($j == 0)
				echo '<td><input type="checkbox" checked><span>' . $modalTags[$i + $nn] . '</span></td>';
			$j = 0;
		}
		else {
			echo '	<td></td>';
		...
		if (isset($modalTags[$i + 2 * $nn])) {
			foreach ($gridConf['tags'] as $tag) {
				if ($tag[0] == $modalTags[$i + 2 * $nn] && $tag[1] == 0) {
					echo '<td><input type="checkbox"><span>' . $modalTags[$i + 2 * $nn] . '</span></td>';
					$j = 1;
				}
			}
			if ($j == 0)
				echo '<td><input type="checkbox" checked><span>' . $modalTags[$i + 2 * $nn] . '</span></td>';
			$j = 0;
		}
		else {
			echo '	<td></td>';
